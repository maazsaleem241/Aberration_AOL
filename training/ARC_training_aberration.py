"""ARC training script -- physics-informed composite loss + curriculum
...
H5_FILE = "/home/awais/Desktop/Maaz/Maaz Data/dataset_c/data/reflection_matrix_dataset.h5"
GRID_SIZE = 40
...
# FIX (do not revert): after each curriculum transition (including the very
# first stage start), linearly ramp LR up to INITIAL_LR over this many
# epochs instead of an instant full-strength jump. Confirmed empirically: an
# instant reset straight onto a fresh, harder data distribution (LR
# 2.5e-4 -> 1e-3 exactly at an aberration_low -> aberration_med transition)
# produced a non-finite loss within 15 batches -- the sudden LR jump hitting
# a suddenly larger gradient/loss magnitude, combined with Adam's momentum
# buffers still being calibrated to the easier stage's smaller gradients.
WARMUP_EPOCHS = 3

USE_AMP = torch.cuda.is_available() 
DEVICE = "cuda" if USE_AMP else "cpu"
BASE_OUTPUT_DIR = "training/ARC_Training/aberration/ARC_training_aberration_new_dataset"
...
# Curriculum stages, exactly matching dataset_generator.py's difficulty_tags
# for the TRAIN split of dataset_c (mode "C", the aberration curriculum --
# see dataset_c's design notes: aberration_multiplier is the ramped axis here,
# with a randomized baseline intensity_ratio in [0,4) present throughout so
# the model still gets some noise-robustness exposure alongside it).
CURRICULUM_STAGES = ["aberration_low", "aberration_med", "aberration_high", "aberration_extreme"]
...
STAGE_TO_VAL_TAG = {
    "aberration_low": "val_aberration_low",
    "aberration_med": "val_aberration_med",
    "aberration_high": "val_aberration_high",
    "aberration_extreme": "val_aberration_extreme",
}

# For early stop incase the loss does not improve by 0.1%
PATIENCE_LIMIT = 15
MIN_DELTA_PERCENT = 0.001   # require a genuine >0.1% relative improvement
...
# loss weights.
# Image-domain supervision added without changing the existing aberration-loss
# weights: SSIM = 0.5 and foreground-weighted L1 = 0.1.
LOSS_WEIGHTS = {
    "coherent": 1.0,
    "local": 1.0,
    "strehl": 0.5,
...
class PhysicsInformedLoss(nn.Module):
    ...
    @staticmethod
    def _reconstruct_from_R_c(
        R_c: Tensor,
        grid_size: int,
        bin_id: Tensor,
        num_bins: int,
    ) -> Tensor:
        """Differentiable 2D Δk-based CASS-style reconstruction.

        All corrected reflection-matrix entries are coherently accumulated
        into their corresponding momentum-transfer bin, then reshaped to the
        object spectrum, inverse-shifted, inverse Fourier transformed, and
        converted to intensity. This replaces the single-column image
        reconstruction for the image-domain training objective.
        """
        b = R_c.shape[0]
        real = R_c.real.reshape(b, -1)
        imag = R_c.imag.reshape(b, -1)
        ...
        object_real = torch.zeros(
            b, num_bins, device=R_c.device, dtype=real.dtype
        )
        object_imag = torch.zeros(
            b, num_bins, device=R_c.device, dtype=imag.dtype
        )

        object_real.index_add_(1, bin_id, real)
        object_imag.index_add_(1, bin_id, imag)
    ...
    def _ssim_loss(
        ...
        Uses a uniform averaging kernel, matching ARC_training_noise.py.
        This gives a differentiable image-domain loss without requiring an
        external SSIM package. Returns a scalar.
        """
        kernel = torch.ones(
    ...
    def _foreground_weighted_l1_loss(
        ...
        """Small auxiliary image loss emphasizing target foreground."""
    ...
    def forward(
        ...
        # Image-domain supervision uses the same predicted corrected matrix
        # R_c and the full 2D Δk/CASS-style reconstruction for SSIM and L1.
        if target_object is not None and (self.w_image > 0 or self.w_image_l1 > 0):
            img_pred = self._reconstruct_from_R_c(
                R_c,
                self.grid_size,
                self.bin_id,
                self.num_bins,
...
def save_visualization(
    ...
    Row 3 reconstruction method matches dataset_generator.py's own
    "Uncorrected Image Intensity" panel: pull the on-axis (center pupil
    coordinate) column of the matrix, reshape to a 2D k-space map, and
    inverse-FFT into a real spatial image. dc_idx uses the CENTERED
    pupil-coordinate convention that phi_i/phi_o themselves use -- unrelated
    to, and unaffected by, the internal raw-FFT convention fixed earlier in
    zernike.py's construct_scattering_matrix (two separate index conventions,
    serving two separate purposes). The corrected image uses the model's own
    predicted phasors to build R_c = P_o^dagger R P_i^dagger -- the exact same
    construction PhysicsInformedLoss uses internally for the Toeplitz term, so
    this panel is an honest "what does the network's current correction
    actually do to the image" check, not a separate metric. Verified
    numerically: feeding the TRUE aberration into this same formula recovers
    the clean object reconstruction exactly.
    """
    ...
    corrected_image = reconstruct_image(R_c)
    ...
    im9 = axes[3, 0].imshow(ground_truth_object, cmap="grey", interpolation="nearest")
    ...
    axes[3, 2].set_title("Model-Corrected Image")
...
def run_training() -> None:
    ...
    epochs_into_stage = 0  # drives the warmup ramp; incremented at the start of each epoch, reset at each curriculum transition
    ...
    print(f"--> Starting ARC curriculum training on {DEVICE.upper()}...")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        train_component_sums = {"coherent": 0.0, "local": 0.0, "strehl": 0.0, "toeplitz": 0.0, "image": 0.0, "image_l1": 0.0}
        n_train_batches = 0
        data_wait_time = 0.0
        compute_time = 0.0
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if not torch.isfinite(loss):
                print(
                    ...
                    f"{float(loss.detach().cpu())}. Skipping this batch -- optimizer NOT "
                    "stepped, no gradient applied. If this recurs frequently, LR, "
                    "WARMUP_EPOCHS, or GRAD_CLIP likely need adjusting."
                )
                optimizer.zero_grad(set_to_none=True)
                t_batch_start = time.time()
                continue
        ...
        with open(csv_path, "a", newline="") as f:
            ...
            row += [avg_train_components[c] for c in ("coherent", "local", "strehl", "toeplitz")]
            row += [stage_val_loss]
            row += [stage_val_components[c] for c in ("coherent", "local", "strehl", "toeplitz")]
            row += [full_val_loss]
            row += [full_val_components[c] for c in ("coherent", "local", "strehl", "toeplitz")]
        ...
        if patience_counter >= PATIENCE_LIMIT:
            if current_stage_idx < len(CURRICULUM_STAGES) - 1:
                ...
                # FIX (do not revert): recreating the scheduler alone only resets
                # its internal plateau-tracking state -- it does NOT touch the
                # optimizer's actual LR value, which previously kept whatever it
                # had decayed down to and continued decaying further from there,
                # monotonically, for the rest of the entire run. Confirmed
                # empirically: LR hit its floor (LR_SCHEDULER_MIN_LR) partway
                # through noise_med and never moved again, so noise_high received
                # essentially zero effective learning rate for its entire run.
                #
                # FIX (do not revert, supersedes the direct-jump version): an
                # INSTANT reset straight to INITIAL_LR was itself confirmed to
                # cause a non-finite loss within 15 batches of a transition --
                # a sudden full-strength LR hitting the new, harder stage's
                # larger gradients, combined with Adam's momentum buffers still
                # calibrated to the easier stage. Resetting epochs_into_stage
                # to 0 here hands control to the WARMUP_EPOCHS ramp at the top
                # of the loop instead, which reaches the exact same INITIAL_LR
                # target but gradually, over WARMUP_EPOCHS epochs, rather than
                # in one step.
                epochs_into_stage = 0
...
def evaluate(
    ...
    total_loss = 0.0
    component_sums = {"coherent": 0.0, "local": 0.0, "strehl": 0.0, "toeplitz": 0.0, "image": 0.0, "image_l1": 0.0}
    n_batches = 0
    for batch in loader:
        batch = move_batch_to_device(batch, DEVICE)
        r, rrh, rhr, R_complex = build_model_inputs(batch)
